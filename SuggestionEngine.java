...
public class SuggestionEngine extends Java8BaseListener {
	...
	public void enterMethodDeclaration(Java8Parser.MethodDeclarationContext ctx) {
		...
		for (int i = 0; i < ctx.methodModifier().size(); i++) { //examining all modifiers
			if (ctx.methodModifier(i).getText().equals("public")) {
	...
	private TreeSet<Candidate> getTopKNeighbor(String word, int K) {
		...
		for (int i = 0; i < mMethods.size(); i++){
			LOGGER.info("Calculating distance for " + mMethods.get(i));
			double distance = Levenshtein.distance(word, mMethods.get(i));
			Candidate x = new Candidate(mMethods.get(i), distance);
			LOGGER.info("Distance is " + distance + " for " + mMethods.get(i));
			if (minHeap.size() < K){
				minHeap.add(x);
			} 
			else {
				Candidate last = minHeap.last();
				if (distance < last.distance){
					minHeap.pollLast(); //remove last (highest) element
					minHeap.add(x);
				}
			}
		}
		LOGGER.info(K + "Methods are selected!");
		return minHeap;
	}
}
